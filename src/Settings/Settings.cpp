...
//
// entocoin is free software: you can redistribute it and/or modify
// it under the terms of the GNU Lesser General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// entocoin is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU Lesser General Public License for more details.
//
// You should have received a copy of the GNU Lesser General Public License
// along with entocoin.  If not, see <http://www.gnu.org/licenses/>.
...
const char DEFAULT_WALLET_FILE_NAME[] = "entocoin.wallet";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultPoolList << "stratum01.cryptoentocoin.cl:9992";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("entocoinwallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("entocoinwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("entocoinwallet.desktop");
  ...
  res = autorunSettings.contains("entocoinWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("entocoinWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.entocoin.entocoinwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=entocoin Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("entocoinWallet", appPath);
    } else {
      autorunSettings.remove("entocoinWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\entocoin", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:entocoin");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\entocoin\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\entocoin\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("entocoinwallet.cfg"));
