...
WalletApplication::WalletApplication(int& _argc, char** _argv) : QApplication(_argc, _argv), m_lockFile(nullptr),
  ...
  setApplicationName("entocoinWallet");
...
bool WalletApplication::init() {
  ...
  m_lockFile = new QLockFile(Settings::instance().getDataDir().absoluteFilePath("entocoincoinwallet.lock"));
  ...
  if (!m_lockFile->tryLock()) {
    WalletLogger::warning(tr("[Application] entocoin wallet already running"));
    if (!paymentUrl.isValid()) {
      QMessageBox::warning(nullptr, QObject::tr("Fail"), "entocoin wallet already running");
    }
...
bool WalletApplication::initCryptoNoteAdapter() {
  for (;;) {
    ...
    case INodeAdapter::INIT_DB_IN_USAGE: {
      ...
      "If you have entocoind with non-default RPC port, you should terminate it and relaunch entocoinWallet\n"
      "or\n"
      "Set the Local deamon required port in entocoinWallet Menu/Preferences/Connection settings."));
	  if (m_splash != nullptr)
		  m_splash->hide();
      dlg.exec();
      return false;
